...
# To lauch this code: Start-Process -FilePath "C:\Program Files\SimVascular\SimVascular\2023-03-27\sv.bat"  -NoNewWindow -ArgumentList "--python", "-- C:\Users\franc\Documents\Code_python\dcm2ctgr\angle_between_contours.py"
# ######################################
...
def generate_contour(cp, id, time_step, path, curve_id):
    ...
    pos.set('x', str(pos_data[0]))
    pos.set('y', str(pos_data[1]))
    pos.set('z', str(pos_data[2]))
...
def rtss2pth(contour_data, roi_name, output_path, scaling_factor = 1.0):
    ...
    # Le code suivant ne fonctionne pas. Le Pyhton API de SimVascular est encore en beta et lessv.segmentation.Series() 
    #n'a pas toutes les fonctions implémentées comme sv.pathplanning.Series().
    # Pour voir la documentation, utiliser import sv puis help(sv.segmentation.Series())
...
def rtss2ctgr(contour_data, roi_name, roi_id, output_path, scaling_factor = 1.0):
    ...
    path = rtss2pth(contour_data, roi_name, output_path)

    # Create a segmentation.
    contour_group = Element("contourgroup")
    contour_group.set("path_name", roi_name)
    ...
    for id, cp in contour_data[roi_name].items():
        ...
        generate_contour(cp, id, time_step, path, curve_id)
        
    #Save as .ctgr file
    xml_ctgr = parseString(tostring(contour_group, encoding='UTF-8')).toprettyxml(indent="   ")
    with open(os.path.join(output_path["ctgr"], roi_name + ".ctgr"), 'w') as f:
        f.write(xml_ctgr)


absolute_path = os.path.dirname(__file__)
contour_data_path = os.path.join(absolute_path, "contour_data.csv")
...
for roi_id, roi_name in enumerate(contour_data.keys()):
    rtss2ctgr(contour_data, roi_name, roi_id, output_path)